...
class StockData:
    ...
    def get_data(ticker, period="2y"):
        try:
            ...
            st_df2 = ta.supertrend(df['High'], df['Low'], df['Close'], length=10, multiplier=2)
            ...
            # Squeeze momentum
            try:
                sq = ta.squeeze(df['High'], df['Low'], df['Close'], df['Volume'])
                df = pd.concat([df, sq], axis=1)
            except:
                pass

            df = pd.concat([df, macd, st_df, st_df2, bb, kc, adx_df, stoch, ichimoku_df], axis=1)
...
if ticker_final:
    ...
    if df is not None and not df.empty:
        ...
        # ────────────────────────────────────────────────────
        with tab1:
            bb_upper = get_col(df, ["BBU_"])
            bb_middle = get_col(df, ["BBM_"])
            bb_lower = get_col(df, ["BBL_"])
            st_val_col = get_col(df, ["SUPERT_7"])
            st_dir_col = get_col(df, ["SUPERTd_7"])
            macd_col = get_col(df, ["MACD_"])
            macdh_col = get_col(df, ["MACDh_"])
            macds_col = get_col(df, ["MACDs_"])
            ...
            # VWAP
            if show_vwap and 'VWAP_D' in df.columns:
                fig.add_trace(go.Scatter(x=df.index, y=df['VWAP_D'], name="VWAP", line=dict(color='#9B59B6', width=1.5, dash='dash')), row=1, col=1)
        ...
        with tab2:
            ...
            with s_col1:
                ...
                st_dir_c = get_col(df, ["SUPERTd_7"])
                st_dir = last.get(st_dir_c, 0) if st_dir_c else 0
                macd_h = last.get(get_col(df, ["MACDh_"]) or 'X', 0)
